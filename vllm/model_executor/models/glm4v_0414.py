...
Glm4vImageInputs = Union[Glm4vImagePixelInputs,
...
Glm4vVideoInputs = Union[Glm4vVideoPixelInputs,
...
class Glm4vVisionPatchMerger(nn.Module):
    ...
    def __init__(
        ...
        self.proj = ColumnParallelLinear(self.hidden_size, self.hidden_size, bias=bias)
        ...
        self.up_proj = ColumnParallelLinear(self.hidden_size,
                                            ...
                                            quant_config=quant_config,
                                            prefix=f"{prefix}.up_proj")
        self.down_proj = RowParallelLinear(self.hidden_size,
                                           context_dim,
                                           bias=bias,
                                           quant_config=quant_config,
                                           prefix=f"{prefix}.down_proj")
    ...
    def forward(self, x: torch.Tensor):
        x = self.proj(x)
        x = self.extra_activation_func(self.ln_q(x))
        x_gate, _ = self.gate_proj(x)
        x_gate = self.act_fn(x_gate)
        x_up, _ = self.up_proj(x)
        x_down, _ = self.down_proj(x_gate * x_up)
...
class Glm4vVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # For GLM-4V-0414 models, float16 will overflow at last block
...
class Glm4vForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def _process_image_input(
            self,
            image_input: Glm4vImageInputs) -> tuple[torch.Tensor, ...]:
    ...
    def _process_video_input(
            self,
            video_input: Glm4vVideoInputs) -> tuple[torch.Tensor, ...]:
